...
- **`generators/`** — real, working generator scripts:
  ...
  - `create_dsm_reexam_sets.py`, `create_dsm_reexam_answers.py` — Re-Examination (makeup test) generator for students requiring a supplementary attempt.
...
This is the most granular, directly verifiable evidence of the paper's Outcome-Based Education (OBE) claims: every single question in the Term-End Examination is programmatically tagged with its Course Outcome and Bloom's-taxonomy level before the paper is assembled (see `MASTER_ROWS` in `create_dsm_final_sets.py`), and three non-identical, difficulty-matched sets are generated automatically from the same tagged question bank — directly substantiating the manuscript's Phase 3 claim of deterministic, parameterised, CO-aligned assessment generation across the full assessment cycle (M1, M2, and the Term-End Examination), not only the mid-semester tests.
